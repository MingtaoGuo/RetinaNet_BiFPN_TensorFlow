...
def resize_img_and_bbox(img, gt_bbox):
    ...
    if h > w:
        factor = IMG_H / h
        new_h, new_w = IMG_H, int(w * factor)
        pad_h = 0
        pad_w = (IMG_W - new_w) // 2
    else:
        factor = IMG_W / w
        new_h, new_w = int(h * factor), IMG_W
        pad_h = (IMG_H - new_h) // 2
        pad_w = 0
    gt_bbox = gt_bbox * factor
    new_img = np.array(Image.fromarray(img).resize([new_w, new_h]))
    pad_img = np.zeros([IMG_H, IMG_W, 3])
    pad_img[pad_h:pad_h + new_h, pad_w:pad_w + new_w] = new_img

    new_y = gt_bbox[:, 1:2] + pad_h
    new_x = gt_bbox[:, 0:1] + pad_w
    new_h = gt_bbox[:, 3:4]
    new_w = gt_bbox[:, 2:3]
    new_bbox = np.concatenate((new_x, new_y, new_w, new_h), axis=1)
    return pad_img, np.int32(new_bbox)
...
def recover_ImgAndBbox_scale(img, pre_bbox):
    ...
    if h > w:
        ...
        pad_w = 0
    pre_bbox = pre_bbox
    x1 = (pre_bbox[0] - pad_w) / factor
    y1 = (pre_bbox[1] - pad_h) / factor
    x2 = (pre_bbox[2] - pad_w) / factor
    y2 = (pre_bbox[3] - pad_h) / factor
...
# for idx, filename in enumerate(filenames):
#     try:
#         IMG, GTBBOX, LABEL = read_data(XML_PATH + filename, IMG_PATH + filename[:-3] + "jpg")
#         IMG, GTBBOX = resize_img_and_bbox(IMG, GTBBOX)
#         IMG, GTBBOX = data_augmentation(IMG, GTBBOX)
#         x, y, w, h = GTBBOX[0, 0], GTBBOX[0, 1], GTBBOX[0, 2], GTBBOX[0, 3]
#         x1, y1, x2, y2 = x - w // 2, y - h // 2, x + w // 2, y + h // 2
#         # img = cv2.rectangle(IMG, (x - w // 2, y - h // 2), (x + w // 2, y + h // 2), (0, 255, 0),
#         #                     2)  # draw_bbox(IMG, [x1, y1, x2, y2], "a")
